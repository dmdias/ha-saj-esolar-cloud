...
def extract_date(date_str, timezone = None):
    try:
        ...
        if timezone:
            ...
            date_obj = datetime.datetime.strptime(date_str + " " + time.strftime('%z'), "%Y-%m-%d %H:%M:%S %z")
    except ValueError:
        try:
            date_obj =  datetime.datetime.strptime(date_str + " " + time.strftime('%z'), "%d/%m/%Y %H:%M:%S %z")
        except ValueError:
            return None

    # Az aktuális időpont meghatározása
    now = datetime.datetime.now(date_obj.tzinfo)  # Az időzónát megtartjuk
...
def is_today(date_string, date_format="%Y-%m-%d %H:%M:%S"):
    try:
        ...
        return date_obj == datetime.datetime.today().date()
    except ValueError:
        return False
